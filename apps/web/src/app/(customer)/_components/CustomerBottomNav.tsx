...
const ITEMS: readonly BottomNavItem[] = [
  ...
  { href: "/orders", label: "Orders", icon: <ClipboardList className="size-5" /> },
  { href: "/orders/history", label: "History", icon: <History className="size-5" /> },
  { href: "/profile", label: "Profile", icon: <User className="size-5" /> },
];

export function CustomerBottomNav() {
  const pathname = usePathname();
  return <BottomNav items={ITEMS} activeHref={pathname} LinkComponent={Link} />;
}
