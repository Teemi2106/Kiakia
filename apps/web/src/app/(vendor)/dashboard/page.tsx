...
const ACTIVE_STATUSES = ["placed", "accepted", "preparing", "ready_for_pickup", "rider_assigned", "picked_up", "in_transit", "arrived"] as const;
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <Card>
          ...
          <CardBody>
            ...
            <p>{vendor.is_accepting_orders ? "Accepting orders" : "Not accepting orders"}</p>
          </CardBody>
        </Card>
        ...
        <Card>
          ...
          <CardBody>
            <p className="text-3xl font-semibold text-ink">{activeCount ?? 0}</p>
          </CardBody>
        </Card>
      </div>
        ...
        <div className="mt-3 flex flex-col gap-3">
          {incoming.map((order) => (
            <Card key={order.id}>
              ...
              <Link href={`/dashboard/orders/${order.id}`} className="mt-2 block text-center text-xs font-medium text-brand-600 hover:underline">
                View details →
              </Link>
            </Card>
          ))}
