...
export function HistoryCard({ order }: HistoryCardProps) {
  ...
  return (
    <div
      className={`rounded-xl border border-[#E4BEB8] bg-white p-4 transition-all hover:shadow-md ${
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center">
        ...
        <div className="mt-2 flex gap-2 md:mt-0 md:flex-col">
          {!isCancelled && !isRejected && (
            <button className="flex-1 rounded-xl border-2 border-[#934B00] px-4 py-2 text-sm font-medium text-[#934B00] transition-colors hover:bg-[rgba(147,75,0,0.05)]">
              Reorder
            </button>
          )}
          <button className="flex-1 rounded-xl px-4 py-2 text-sm font-medium text-[#5B403C] transition-colors hover:bg-[#F0EDED]">
            Details
          </button>
        </div>
      </div>
